...
  <div className="flex justify-center my-4"> {/* Added spacing */}
    {[...Array(5)].map((_, i) => (
      <svg 
        key={i} 
        // Style badal diya gaya hai: text-white aur size w-7 h-7
        className={`w-7 h-7 ${i < rating ? "text-white" : "text-white opacity-40"}`} 
        fill="currentColor" 
        viewBox="0 0 20 20"
      >
...
              >
                {/* === CARD STYLING START === */}
                <div className="relative bg-yellow-400 rounded-3xl h-full flex flex-col items-center justify-center p-8 text-center shadow-lg shadow-yellow-800/20 transition-all duration-300 hover:shadow-xl hover:shadow-yellow-800/30">
                  
                    ...
                    <div className="absolute -top-14 left-1/2 -translate-x-1/2 z-20">
                      <div className="w-28 h-28 rounded-full p-1 bg-white shadow-lg">
                        <img 
                          src={review.avatar} 
                          alt={review.name} 
                          className="w-full h-full rounded-full object-cover border-4 border-yellow-400" // Border card ke bg se match karta hai
                        />
                      </div>
                    </div>

                    {/* Card Content (with padding-top to clear the image) */}
                    <div className="relative z-10 flex flex-col items-center pt-14"> {/* pt-14 image ke liye jagah banata hai */}
                        
                        {/* Star rating upar aa gaya hai */}
                        <StarRating rating={review.rating} />
                        
                        <p className="font-bold text-2xl text-gray-900 font-serif mt-2"> {/* Text color dark kar diya hai */}
                            {review.name}
                        </p>
                        
                        {/* Location ke saath icon add kiya hai */}
                        <p className="text-sm text-gray-800 mb-4 flex items-center gap-1">
                          <FaMapMarkerAlt className="text-red-600" />
                          {review.location}
                        </p>
                        
                        <p className="mt-4 text-gray-800 text-base md:text-lg leading-relaxed"> {/* Text color dark kar diya hai */}
                            {review.text}
              Bha           </p>
                    </div>
                </div>
